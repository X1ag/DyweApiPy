...
async def writeFloorInFile(data, address: str = "EQAOQdwdw8kGftJCSFgOErM1mBjYPe4DBPq8-AhF6vr9si5N", timeframe='5m'):
...
async def getData(address: str = "EQAOQdwdw8kGftJCSFgOErM1mBjYPe4DBPq8-AhF6vr9si5N", timeframe='5m'):
    while True:
        try:
            data = {
                'openTime': int(get_time(filename=f'./candles/candles{address}{timeframe}.json')[0].timestamp() * 1000),
                'closeTime':int(get_time(filename=f'./candles/candles{address}{timeframe}.json')[1].timestamp() * 1000),
                'percentChangePrice': percentChange(),
...
if __name__ == "__main__":
    ...
    asyncio.run(main(address, timeframe="5m"))